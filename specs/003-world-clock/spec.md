...
- **FR-002**: 系統 MUST 在頁面正中央顯示一個主要時間區域，預設顯示本地時間（台北 GMT+8）
...
- **FR-007**: 系統 MUST 在主要時間區域的右下角顯示當前日期（格式：YYYY-MM-DD）
- **FR-008**: 日期 MUST 根據主要時間顯示區域所選定的時區來計算和顯示
- **FR-009**: 系統 MUST 實作心跳檢測機制：每分鐘檢查上次更新時間戳，若發現超過 5 秒未更新，則自動清除舊計時器並重新啟動，同時記錄錯誤到瀏覽器 console
...
- **FR-013**: 系統 MUST 在使用者將滑鼠移至城市時間卡片上方時提供視覺回饋（背景色變為 #e9ecef）
- **FR-014**: 系統 MUST 在使用者點選城市時間卡片時提供點選動畫效果（顯示藍色邊框 #007bff）
...
- **FR-022**: 當瀏覽器的 Intl.DateTimeFormat API 無法偵測時區或發生錯誤時，系統 MUST 顯示「無法偵測時區，請重新整理頁面」的錯誤訊息，並提供重新整理按鈕
- **FR-023**: 系統 MUST 支援以下瀏覽器版本：Chrome/Edge 90+, Firefox 88+, Safari 14+（最新 2 個主要版本）
- **FR-024**: 當偵測到不支援的瀏覽器版本時，系統 MUST 顯示瀏覽器升級建議訊息

#### 視覺設計

- **FR-025**: 系統 MUST 使用以下色彩配置：
  - 主背景：#f8f9fa（淺灰）
