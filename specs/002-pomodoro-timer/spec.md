...
2. **Given** 工作時段計時器倒數至 00:00, **When** 時間結束, **Then** 系統顯示通知訊息並自動切換到休息時段（05:00）
3. **Given** 休息時段計時器倒數至 00:00, **When** 時間結束, **Then** 系統顯示完成訊息，番茄鐘計數增加 1
...
1. **Given** 工作時段計時器正在執行, **When** 時間倒數進行, **Then** 進度指示器同步更新，顯示已使用時間的百分比
2. **Given** 計時器顯示進度, **When** 從工作時段切換到休息時段, **Then** 進度指示器重置並開始顯示休息時段的進度
3. **Given** 使用者暫停計時器, **When** 檢視進度指示器, **Then** 進度停留在暫停時的位置
4. **Given** 使用者自訂時長後啟動計時, **When** 時間倒數, **Then** 進度指示器根據自訂時長正確計算並顯示百分比
...
- **資料損壞**: 儲存資料損壞或格式錯誤時，系統如何處理並恢復到預設狀態？
- **極端自訂值**: 使用者輸入無效的時長（例如 0 分鐘、負數或超大數值）時，系統如何驗證並防止錯誤？
- **同時開啟多個視窗**: 使用者在多個視窗同時開啟計時器頁面，資料同步和計時狀態如何處理？
- **跨日界計時**: 如果工作時段從 23:50 開始，跨越午夜到新的一天，番茄鐘計數應該記錄在哪一天？
...
- **FR-010**: 系統必須在時段結束時顯示通知訊息，告知使用者階段切換
...
**Impact**: 中  
**Probability**: 低  
**Mitigation**: 在 MVP 階段接受此限制，提示使用者僅在單一視窗使用。未來可實作跨視窗狀態同步機制。
